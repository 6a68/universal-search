...
Search.prototype = {
  ...
  loadIntoWindow: function(win) {
    ...
    this._reloadUrlbar(win);
    // theory 1: we need to reload the urlbar before altering the bindings
    // nope: we see an unstyled popup. like our css isn't even applied to the DOM yet.
    console.error('is the stylesheet in the DOM?', win.document.getElementById('UniversalSearchStyleSheet')); 
    let popup = win.document.getElementById('PopupAutoCompleteRichResult');
    popup.id = 'USearchPopupAutoCompleteRichResult';
    // theory 2: we need to alter the bindings then reload the urlbar
    // theory 3: we need to wait for something else to happen--why does it work in the debugger?
