...
HighlightManager.prototype = {
  ...
  attachMouseListeners: function() {
    // This function gathers all events used to manage highlight state in
    ...
    // When the user mouses out of the recommendation, if they are leaving
    // the popup, keep the highlight, in line with current mouseout behavior
    // in the results list. If the mouse is moving to the results list, remove
    // the highlight from the recommendation.
    ...
    // the highlight, and should lose the highlight. (The user might mouse
    // directly into the results list from outside the popup.)
  ...
  onRecommendationMouseEnter: function() {
    this.win.console.log('mouse enter');
    this.popup.richlistbox.selectedIndex = -1;
    this.recommendation.isHighlighted = true;
  },
  onRecommendationMouseMove: function() {
    // TODO: should we return if isHighlighted? or just naively update the DOM?
    // which has lower impact?
    this.win.console.log('mouse move');
    this.popup.richlistbox.selectedIndex = -1;
    this.recommendation.isHighlighted = true;
  },
  onRecommendationMouseLeave: function(evt) {
    ...
    // even though the pointer may be over other parts of the XUL DOM.
